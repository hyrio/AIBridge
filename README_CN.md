...
可在 `AIBridge/Settings > Runtime` 配置默认开关、Editor Play Mode 自动注入、Development Build 自动注入、Release Build 允许项、后台保持运行、Runtime 目录、TargetId、Auth Token、Allowed Actions 和日志缓存。`AIBridge/Players` 面板可查看当前 heartbeat 可发现的 Players、状态、场景、平台和常用 CLI 命令。Editor Play Mode 自动注入默认启用，进入 Play Mode 且场景内没有 `AIBridgeRuntime` 时会创建临时隐藏 Runtime 对象。`后台保持运行` 默认对 Editor Play Mode 和 Development Build 启用，避免 Unity 失焦后 heartbeat 和 runtime 命令停止响应。
